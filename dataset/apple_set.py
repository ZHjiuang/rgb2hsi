...
class HS_multiscale_DSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img1 = spectral.envi.open(imgpath.split('.')[0]+'.hdr', imgpath)
        samples = np.linspace(0, img1.nbands - 1, self.opt.out_channels, dtype=int)  # 指定dtype为int，因为我们需要整数采样点

        spectral_img = img1.read_bands(samples)  # img1.nbands
        img = img1.read_bands([58, 91, 174])  # 分别对应了1064.8 1175.8 1449nm波段
